...
void setup()
{
  ...
  for (int i = 0; i < rigAnim.length; i++) {
    rigAnim[i] = new Anim();
    rigAnim[i].setupAnim();
  }
...
void draw()
{
  ...
  //}
  for (int i = 0; i < rigAnim.length; i++) {
    if (beatCounter % rigAnim.length == i) {
      rigAnim[i].trigger(beatDetect.isOnset());
    }
    blendMode(LIGHTEST);
    rigAnim[i].drawAnim(rigAnim[i].window, size.rigWidth/2+(i*50), size.rigHeight/2+(i*50));
  }
  blendMode(MULTIPLY);
