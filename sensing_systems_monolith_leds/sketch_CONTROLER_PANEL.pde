...
class MainControlFrame extends ControlFrame {
  ...
  void draw() {
    ...
    ///////////// rig info/ ///////////////////////////////////////////////////////////////////
    fill(da.flash, 300);
    if (!da.toggle) fill(da.c, 100);
    text("rigViz: " + da.availableAnims[da.vizIndex], x, y);
    text("bkgrnd: " + da.availableBkgrnds[da.bgIndex], x, y+20);
    text("func's: " + da.availableFunctionEnvelopes[da.functionIndexA] + " / " + da.availableFunctionEnvelopes[da.functionIndexB], x+110, y);
    text("alph's: " + da.availableAlphaEnvelopes[da.alphaIndexA] + " / " + da.availableAlphaEnvelopes[da.alphaIndexB], x+110, y+20);
    ...
    sec = nf(int(colorTime*60 - (millis()/1000 - da.colorTimer)) %60, 2, 0);
    min = int(colorTime*60 - (millis()/1000 - da.colorTimer)) /60 %60;
    text("next color in: "+ min+":"+sec, x, y+20);
    //text("c-" + da.colorIndexA + "  " + "flash-" + da.colorIndexB, x, y+40);
    /////////////////////////////////////////////////// ro info ////////////////////////////////////////////////////////
    if (size.roWidth > 0 && size.roHeight > 0) {
      fill(da.c, 300);
      if (!ro.toggle) fill(da.c, 100);
      textSize(18);
      textAlign(LEFT);
      x = size.ro.x-(size.roWidth/2)+10;
      text("roViz: " + ro.availableAnims[ro.vizIndex], x, y);
      text("bkgrnd: " + ro.availableBkgrnds[ro.bgIndex], x, y+20);
      text("func's: " + ro.availableFunctionEnvelopes[ro.functionIndexA] + " / " + ro.availableFunctionEnvelopes[ro.functionIndexB], x+120, y);
      text("alph's: " + ro.availableAlphaEnvelopes[ro.alphaIndexA] + " / " + ro.availableAlphaEnvelopes[ro.alphaIndexB], x+120, y+20);
    }
    /////////////////////////////////////////////////// pa info ////////////////////////////////////////////////////////
    if (size.paWidth > 0 && size.paHeight > 0) {
      fill(da.c, 300);
      if (!pa.toggle) fill(da.c, 100);
      textSize(18);
      textAlign(LEFT);
      x = size.pa.x-(size.paWidth/2)+10;
      text("liveViz: " + pa.availableAnims[pa.vizIndex], x, y);
      text("bkgrnd: " + pa.availableBkgrnds[pa.bgIndex], x, y+20);
      text("func's: " + pa.availableFunctionEnvelopes[pa.functionIndexA] + " / " + pa.availableFunctionEnvelopes[pa.functionIndexB], x+120, y);
      text("alph's: " + pa.availableAlphaEnvelopes[pa.alphaIndexA] + " / " + pa.availableAlphaEnvelopes[pa.alphaIndexB], x+120, y+20);
    }

    /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    int sliderY =90;
    sequencer(da.wide, sliderY-20);
    pauseInfo(width-5, sliderY-15);
    dividerLines();
    fill(da.c);                              // divider for sliders
...
/*
    try {
      for (Anim anim : da.animations) {
        if (i<da.animations.size()-1) {
          fill(da.c1, 120);
        } else {
          fill(da.flash1, 300);
        }
    ...
    */
    ...
    textAlign(LEFT);
    textSize(18);
    text("alph A : "+da.alphaIndexA, 12, y-12);
    text("alph B : "+da.alphaIndexB, this.width/2+12, y-12);
    rectMode(CORNER);
    rect(12, y - 5, 1, y1-25);
    ...
    fill(da.c1, 200);
    text("func A : "+da.functionIndexA, 12, y-12+y1);
    text("func B : "+da.functionIndexB, this.width/2+12, y-12+y1);
    rectMode(CORNER);
...
class ControlFrame extends PApplet {
  ...
  void dividerLines() {
    fill(da.c, 200);                         // box around the outside
