...
class OPCGrid {
  ...
  PVector[] paString = new PVector[3];
  PVector[] pa = new PVector[18];
  ...
  float mirrorAndGap, seedsLength, _seedsLength, seeds2Length, _seeds2Length, paLength, _paLength, _mirrorWidth, mirrorWidth, controllerWidth;
  ...
  void mirrorsOPC(OPC opc, OPC opc1, int gridStep) {
    ...
    //da.position=opcGrid.mirror;
    //da.positionX=opcGrid.mirrorX;
  }
  ////////////////////////////////////// MIRROR TEST ///////////////////////////////////////////////////////////////////
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  void tawSeedsOPC(Rig _rig, OPC opc, OPC opc1) {
    ...
    int pd = int(_seedsLength/seedLeds); //int(size.roWidth/seedLeds*1.49);
  ...
  void individualCansOPC(Rig _rig, OPC opc, boolean offset) {
    ...
    for (int i=0; i<pa.length/xw; i++) pa[i] =     new PVector (rig.size.x-(rig.wide/2)+(rig.wide/(pa.length/xw+1)*(i+1)), y-(rig.high/2)+rig.high/(xw+1)*1);
    for (int i=0; i<pa.length/xw; i++) pa[i+3] =   new PVector (rig.size.x-(rig.wide/2)+(rig.wide/(pa.length/xw+1)*(i+1)), y-(rig.high/2)+rig.high/(xw+1)*2);
    for (int i=0; i<pa.length/xw; i++) pa[i+6] =   new PVector (rig.size.x-(rig.wide/2)+(rig.wide/(pa.length/xw+1)*(i+1)), y-(rig.high/2)+rig.high/(xw+1)*3);
    for (int i=0; i<pa.length/xw; i++) pa[i+9] =   new PVector (rig.size.x-(rig.wide/2)+(rig.wide/(pa.length/xw+1)*(i+1)), y-(rig.high/2)+rig.high/(xw+1)*4);
    for (int i=0; i<pa.length/xw; i++) pa[i+12] =  new PVector (rig.size.x-(rig.wide/2)+(rig.wide/(pa.length/xw+1)*(i+1)), y-(rig.high/2)+rig.high/(xw+1)*5);
    for (int i=0; i<pa.length/xw; i++) pa[i+15] =  new PVector (rig.size.x-(rig.wide/2)+(rig.wide/(pa.length/xw+1)*(i+1)), y-(rig.high/2)+rig.high/(xw+1)*6);

    if (offset) {
      pa[1].y = y-(rig.high/2)+rig.high/(xw+1)*(1.5);
      pa[4].y = y-(rig.high/2)+rig.high/(xw+1)*(2.5);
      pa[7].y = y-(rig.high/2)+rig.high/(xw+1)*(3.5);
      pa[10].y = y-(rig.high/2)+rig.high/(xw+1)*(4.5);
      pa[13].y = y-(rig.high/2)+rig.high/(xw+1)*(5.5);
      pa[16].y = y-(rig.high/2)+rig.high/(xw+1)*(6.5);
    }

    int fc = 9 * 512;
    int channel = 64;
    for (int i = 0; i < pa.length/3; i++) opc.led(fc+(channel*0+i), int(pa[i].x), int(pa[i].y));                   
    for (int i = 0; i < pa.length/3; i++) opc.led(fc+(channel*1+i), int(pa[i+6].x), int(pa[i+6].y));                  
    for (int i = 0; i < pa.length/3; i++) opc.led(fc+(channel*2+i), int(pa[i+12].x), int(pa[i+12].y));                  

    ////  set ro position to individual pa positions
    for (int i = 0; i < rig.position.length/2; i++) {
      rig.position[i].x=opcGrid.pa[i].x-(rig.size.x-(rig.wide/2));
      rig.position[i].y=opcGrid.pa[i].y-(rig.size.y-(rig.high/2));
      //
      rig.position[i+6].x=opcGrid.pa[i+12].x-(rig.size.x-(rig.wide/2));
      rig.position[i+6].y=opcGrid.pa[i+12].y-(rig.size.y-(rig.high/2));
    }
    rig.position[4].x=pa[7].x-(rig.size.x-(rig.wide/2));
    rig.position[4].y=pa[7].y-(rig.size.y-(rig.high/2));

    rig.position[7].x=pa[10].x-(rig.size.x-(rig.wide/2));
    rig.position[7].y=pa[10].y-(rig.size.y-(rig.high/2));
  }
  /////////////////////////////////////////////////////////////////////////////////////////////////////
  void kallidaCansOPC(OPC opc) {
    ...
    int leds = 6;
    pd = int(_paLength/6);
    opc.ledStrip(fc+(channel*0), leds, int(paString[0].x), int(paString[0].y), pd, 0, false);                   /////  6 CANS PLUG INTO slot 0 on CANS BOX /////// 
    opc.ledStrip(fc+(channel*1)+(64*channel), leds, int(paString[1].x), int(paString[1].y), pd, 0, false);      /////  6 CANS PLUG INTO slot 1 on CANS BOX ///////
    paLength = _paLength - (pd/2);
  } 
  /////////////////////////////////////////////////////////////////////////////////////////////////////
  void pickleCansOPC(Rig _rig, OPC opc, int fc) {
    rig = _rig;
    _paLength = rig.high/1.2;
    ...
    paString[0] = new PVector(rig.size.x-(rig.wide/3), rig.size.y-(pd/4));
    paString[1] = new PVector(rig.size.x, rig.size.y+(pd/4));
    paString[2] = new PVector(rig.size.x+(rig.wide/3), rig.size.y-(pd/4));

    opc.ledStrip(fc+(channel*1), leds, int(paString[0].x), int(paString[0].y), pd, PI/2, false);                   /////  PLUG INTO slot 1 on CANS BOX (first tail) /////// 
    opc.ledStrip(fc+(channel*2), leds, int(paString[1].x), int(paString[1].y), pd, PI/2, false);                   /////  PLUG INTO slot 2 on CANS BOX /////// 
    opc.ledStrip(fc+(channel*3), leds, int(paString[2].x), int(paString[2].y), pd, PI/2, false);                   /////  PLUG INTO slot 3 on CANS BOX /////// 

    paLength = _paLength - (pd/2);
  } 
  /////////////////////////////////////////////////////////////////////////////////////////////////////
  void sixCansOPC(Rig _rig, OPC opc, int fc, int slot, int offset) {
    rig = _rig;
    _paLength = rig.high/1.2;
    ...
    int channel = 64;
    int leds = 6;
    pd = int(_paLength/leds);
    opc.ledStrip(fc+(channel*slot), leds, rig.size.x+offset, rig.size.y, pd, PI/2, false);                   /////  PLUG INTO slot 1 on CANS BOX (first tail) /////// 
  ...
  void castleCansOPC(Rig _rig, OPC opc, OPC opc1, boolean offset) {
    ...
    int fc = 9 * 512;
    int channel = 64;
    for (int i = 0; i < pa.length/3; i++) opc.led(fc+(channel*0+i), int(pa[i].x), int(pa[i].y)+80);     
    fc = 10 * 512;
    for (int i = 0; i < pa.length/3; i++) opc1.led(fc+(channel*1+i), int(pa[i+6].x), int(pa[i+6].y)+80);                  
    for (int i = 0; i < pa.length/3; i++) opc1.led(fc+(channel*2+i), int(pa[i+12].x), int(pa[i+12].y)+80);                  
  ...
  void kingsHeadCansOPC(Rig _rig, OPC opc) {
    rig = _rig;
    _paLength = size.paWidth;
    paString[0] = new PVector(rig.size.x, rig.size.y-(rig.high/4));
    paString[1] = new PVector(rig.size.x, rig.size.y);
    paString[2] = new PVector(rig.size.x, rig.size.y+(rig.high/4));

    int fc = 0 * 512;
    int channel = 64;
    int leds = 6;
    pd = int(_paLength/6);
    opc.ledStrip(fc+(channel*0), leds, int(paString[0].x), int(paString[0].y), pd, 0, true);                   /////  6 CANS PLUG INTO slot 0 on CANS BOX /////// 
    opc.ledStrip(fc+(channel*1), leds, int(paString[1].x), int(paString[1].y), pd, 0, true);                   /////  6 CANS PLUG INTO slot 1 on CANS BOX /////// 
    opc.ledStrip(fc+(channel*2), leds, int(paString[2].x), int(paString[2].y), pd, 0, true);                   /////  6 CANS PLUG INTO slot 2 on CANS BOX /////// 
    paLength = _paLength - (pd/2);
  }
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  void castleFireplaceCansOPC(Rig rig, OPC opc) {
    int fc = 10 * 512;
    int channel = 64;
    //for (int i = 0; i < 6; i++) opc.led(fc+(channel*2+i), int(te.size.x), int(te.size.y-(te.high/2)+100+(i*80)));
  } 
  /////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  void boothOPC(OPC opc, int fc, int slot) {
    booth = new PVector (104, 15);
    int channel = 64;       
    opc.led(fc+(channel*slot), int(booth.x), int(booth.y));
    opc.led(fc+(channel*slot+1), int(booth.x-10), int(booth.y));
    opc.led(fc+(channel*slot+2), int(booth.x+10), int(booth.y));
  }
  /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  void digOPC(OPC opc, int fc, int slot) {
    dig = new PVector (booth.x+110, booth.y);
    int channel = 64;           
    opc.led(fc+(channel*slot), int(dig.x), int(dig.y));
    opc.led(fc+(channel*slot+1), int(dig.x-10), int(dig.y));
    opc.led(fc+(channel*slot+2), int(dig.x+10), int(dig.y));
  }
  /////////////////////////////////////////////////////////////////////////////////////////////////////
  ...
  ////////////////////////////////// DMX  /////////////////////////////////////////////////////////////////////////////////////
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  void dmxParsOPC(Rig _rig, OPC opc, int fc, int numberOfPars) {
    rig = _rig;
    fc *= 512;
    int gap = rig.high/(numberOfPars+2);
    for (int i = 0; i < numberOfPars; i++) opc.led(6144+i, int(rig.size.x), int(rig.size.y-(gap*numberOfPars/2)+(gap/2)+(i*gap)));
  }
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
