...
void DMXcontrollingUs() {
  ...
  //attach to vizIndex
  if (dmxData[4]>0 ) {
    da.vizIndex=int(map(dmxData[3], 1, 255, 0, da.availableAnims.length-1)+0.5);//plus 0.5 rounds rather than truncates
  }
  //DMX attach color
  if (dmxData[0]>0 || dmxData[1]>0 || dmxData[2]>0) {
    da.c = xcolor(dmxData[0], dmxData[1], dmxData[2]);
    da.flash = xcolor(dmxData[0],dmxData[1],dmxData[2]);
  }
  //DMX attach tdaer
  if (dmxData[7]>0 ) {
    //debounce
    if (tdaertimes[7]<millis()+debouncetime) {
      //da.addAnim(da.availableAnims[da.vizIndex]);
      tdaertimes[7]=millis();
    }
  }
  //DMX attach float
  if (dmxData[8]>0 && false) {
    ro.dimmer = map(dmxData[8], 1, 255, 0, 1);
  }
}
...
void syphonSetup(boolean toggle) {
 ...
 println();
 syphonImageSent = createGraphics(da.wide, da.high, P2D);
