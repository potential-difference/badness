...
class OPCGrid {
  ...
  public void standAloneBoothOPC(OPC opc) {
    ...
    opc.led(fc+(channel*4), PApplet.parseInt(dig.x+5), PApplet.parseInt(dig.y));
  }
  ...
  public void wigflexLanterns(Rig _rig, OPC opc) {
    ...
    ///////////////////////// if you need to make another chain just change the 0 to 1 (or whichever slot the start of the chain is plugged into)
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    opc.led(0, PApplet.parseInt(rig.positionX[3][0].x + Xoffset), PApplet.parseInt(rig.positionX[3][0].y)); 

    opc.led(1, PApplet.parseInt(rig.position[0].x + Xoffset), PApplet.parseInt(rig.position[0].y));       
    opc.led(2, PApplet.parseInt(rig.position[5].x + Xoffset), PApplet.parseInt(rig.position[5].y));      

    opc.led(3, PApplet.parseInt(rig.positionX[2][1].x + Xoffset), PApplet.parseInt(rig.positionX[2][1].y)); 
    opc.led(4, PApplet.parseInt(rig.positionX[4][1].x + Xoffset), PApplet.parseInt(rig.positionX[4][1].y)); 

    opc.led(5, PApplet.parseInt(rig.position[6].x + Xoffset), PApplet.parseInt(rig.position[6].y)); 
    opc.led(6, PApplet.parseInt(rig.position[11].x + Xoffset), PApplet.parseInt(rig.position[11].y));  

    opc.led(7, PApplet.parseInt(rig.positionX[3][2].x + Xoffset), PApplet.parseInt(rig.positionX[3][2].y));
    opc.led(8, PApplet.parseInt(rig.positionX[3][1].x + Xoffset), PApplet.parseInt(rig.positionX[3][1].y)+(rig.high-(PApplet.parseInt(rig.high/1.2f)))/2);
  }
  ////////////////////////////////// DMX  /////////////////////////////////////////////////////////////////////////////////////
  ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
public class Rig {
  ...
  Rig(float _xpos, float _ypos, int _wide, int _high, String _name) {
    ...
    customize(ddAlphaListB, color(bac1, 200), bac, act, PApplet.parseInt(listWidth/2.1f), "alphB");   // customize the list
    customize(ddFuncListA, color(bac1, 200), bac, act, PApplet.parseInt(listWidth/2.1f), "funcA");     // customize the list
    customize(ddAlphaListA, color(bac1, 200), bac, act, PApplet.parseInt(listWidth/2.1f), "alphA");   // customize the list
    ...
    loadSlider( "alphaRate", x+(clm*arrayListIndex), y+(1*row), swide, shigh, 0, 1, 0.3f, act, bac, slider);      // old alphaSlider - controls rate of DECAY for ALPHA
    this.cp5.getController(this.name+" "+"alphaRate").setLabel("alpha rate");
    loadSlider( "funcRate", x+(clm*arrayListIndex), y+(2*row), swide, shigh, 0, 1, 0.4f, act1, bac1, slider1);    // old funcSlider - control rate of DECAY for FUNCTION
    this.cp5.getController(this.name+" "+"funcRate").setLabel("func rate");
    loadSlider( "blurValue", x+(clm*arrayListIndex), y+(3*row), swide, shigh, 0, 1, 0.5f, act, bac, slider);      // blurriness of vizulisation 
    ...
    loadSlider( "manualAlpha", x+(clm*arrayListIndex), y+(10*row), swide, shigh, 0, 1, 0.8f, act1, bac1, slider1);  // RATE of ALPHA DECAY for manual control - needs to be impemented properly
  ...
  public void ddListCallback( ScrollableList ddl, int index) {
    ddl.setValue(index);
  }
  //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
class MainControlFrame extends ControlFrame {
    ...
    public void draw() {
        ...
        if (roof != null) {
            fill(rigg.c, 300);
            // *** TO DO *** // MAKE THIS INDEPENDENT OF RIGG
...
class SliderFrame extends ControlFrame {
    ...
    public void setup() {
        ...
        loadSlider("vizTime", x, y + row * 2, wide, high, 0.5f, 30, 5, act1, bac1, slider1);
        this.cp5.getController("vizTime").setLabel("viz timer");
        
        loadSlider("colorTime", x, y + row * 3, wide, high, 0.5f, 30, 6, act, bac, slider);
        ...
        loadSlider("smokeOnTime", x, y + row * 6.5f, wide / 2, high, 0, 5, 3, act, bac, slider);
        this.cp5.getController("smokeOnTime").setLabel("smoke ON time");
        loadSlider("smokeOffTime", x, y + row * 7.5f, wide / 2, high, 0, 20, 10, act1, bac1, slider1);
        this.cp5.getController("smokeOffTime").setLabel("smoke OFF time");
        loadSlider("smokePumpValue", x, y + row * 8.5f, wide / 2, high, 0, 1, 0.1f, act, bac, slider);
...
public void loadShaders() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "pickle_shields_jan_20" };
    if (passedArgs != null) {
