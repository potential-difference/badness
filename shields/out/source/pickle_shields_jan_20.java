...
///////////////////////////// *** MANUAL ANIM WORK THAT DOESNT WORK **** ////////////////////////////
  ...
  if (millis()-lastTime[49]>debouncetime) {
   ...
   anim.functionEnvelopeA = anim.functionEnvelopeA.mul(0.6f+(stutter*0.4f));  //     anim.functionEnvelopeA.mul((1-cc[54])+(stutter*cc[54]));
   anim.functionEnvelopeB = anim.functionEnvelopeB.mul(0.6f+(stutter*0.4f));    //anim.functionEnvelopeB.mul((1-cc[54])+(stutter*cc[54]));
   }
   lastTime[41]=millis();
   }
  ...
  if (padVelocity[42] > 0) rigg.colorSwap(0.9999999999f);

  if (padVelocity[51] > 0) roof.colorSwap(0.9999999999f);
  if (padVelocity[43] > 0) pars.colorSwap(0.9999999999f);                // COLOR SWAP MOMENTARY
...
class SliderFrame extends ControlFrame {
  ...
  public void setup() {
    ...
    for (int i =0; i<16; i+=2) {
      ...
      loadSlider( name, x, y+(i*gap), wide, high, 0, 1, 0.32f, act1, bac1, slider1);
      loadSlider( name1, x, y+gap+(i*gap), wide, high, 0, 1, 0.32f, act, bac, slider);
    }
