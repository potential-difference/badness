...
public class Rig {
  ...
  ControlP5 cp5;
  PApplet parent;
  ScrollableList ddVizList, ddBgList, ddPalList, ddAlphaList, ddFuncList, ddAlphaListB, ddFuncListB;
  ...
  Rig(boolean _toggle, float _xpos, float _ypos, int _wide, int _high, String _name) {
    ...
    pass1 = createGraphics(wide/5, high/5, P2D);
    pass1.noSmooth();
    pass1.imageMode(CENTER);
    pass1.beginDraw();
    pass1.noStroke();
    pass1.endDraw();
    pass2 = createGraphics(wide/5, high/5, P2D);
    pass2.noSmooth();
    pass2.beginDraw();
    ...
    availableColorsC = new int[] {5, 3, 4, 0, 7};
    availableColorsFlash = new int[] { 13, 11, 12, 8, 15};
    ...
    for (int i=0; i<availableColorsC.length; i++) {
      ...
      cRadioButton.getItem(name+" colc "+i)
        .setColorBackground(color(col[availableColorsC[i]], 100))
        .setColorForeground(color(col[availableColorsC[i]], 200))
    ...
    for (int i=0; i<availableColorsFlash.length; i++) {
      ...
      flashRadioButton.getItem(name+" colFlash "+i)
        .setColorBackground(color(col[availableColorsFlash[i]], 100))
        .setColorForeground(color(col[availableColorsFlash[i]], 200))
        .setColorActive(color(col[availableColorsFlash[i]], 360));
  ...
  void colorTimer(float colTime, int steps) {
    ...
    if (change == true) {
      go = 1;
      colorIndexA =  (colorIndexA + steps) % (availableColorsC.length-1);
      colB =  col[colorIndexA];
      colorIndexB = (colorIndexB + steps) % (availableColorsFlash.length-1);
