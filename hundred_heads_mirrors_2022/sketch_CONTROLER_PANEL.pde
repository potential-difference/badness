...
class MainControlFrame extends ControlFrame {
  ...
  void setup() {
   ...
   pars = new Rig(true, size.pars.x, size.pars.y, size.parsWidth, size.parsHeight, "PARS");
    ...
    loadSlider("smokeOnTime", x, y+row*11, wide/2, high, 1, 10, 3, act, bac, slider);
    loadSlider("smokeOffTime", x, y+row*12, wide/2, high, 0, 20, 10, act1, bac1, slider1);
    loadSlider("smokePumpValue", x, y+row*13, wide/2, high, 0, 1, 0.1, act, bac, slider);
...
class ControlFrame extends PApplet {
  ...
  public void controlEvent(ControlEvent theEvent) {
    ...
    for (Rig rig : rigs) {                        
      ...
      if (theEvent.isFrom(rig.ddPalList)) {
        if (frameCount > someDelay)  println(rig.name+" palette selected "+intValue);
        RadioButton cRadioButton = cp5.get(RadioButton.class,rig.name+" cRadioButton");
        for (int i = 0; i < rig.availableColorsC.length; i++){
          cRadioButton.removeItem(rig.name+" colc "+i);
        }
        rig.availableColorsC = rig.availablePalettes[intValue].colorsC;
        for (int i= 0;i<rig.availableColorsC.length; i++){
          cRadioButton.addItem(rig.name+" colc "+i,i);
          cRadioButton.getItem(rig.name+" colc "+i)
            .setColorBackground(color(rig.col[rig.availableColorsC[i]], 100))
            .setColorForeground(color(rig.col[rig.availableColorsC[i]], 200))
            .setColorActive(color(rig.col[rig.availableColorsC[i]], 360));
          cRadioButton.hideLabels();
        }
        
        RadioButton flashRadioButton = cp5.get(RadioButton.class,rig.name+" flashRadioButton");
        for(int i=0;i<rig.availableColorsFlash.length;i++){
          flashRadioButton.removeItem(rig.name+" colFlash "+i);
        }
        rig.availableColorsFlash = rig.availablePalettes[intValue].colorsFlash;
        for (int i=0; i<rig.availableColorsFlash.length; i++) {
          flashRadioButton.addItem(rig.name+" colFlash "+i, i);
          flashRadioButton.getItem(rig.name+" colFlash "+i)
            .setColorBackground(color(rig.col[rig.availableColorsFlash[i]], 100))
            .setColorForeground(color(rig.col[rig.availableColorsFlash[i]], 200))
            .setColorActive(color(rig.col[rig.availableColorsFlash[i]], 360));
